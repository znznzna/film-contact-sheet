...
class ContactSheet:
    ...
    # A4サイズ (mm -> pixel at 300dpi) - 最大サイズとして使用
    A4_WIDTH_MM = 210
    A4_HEIGHT_MM = 297
    DPI = 300
    
    def __init__(self):
        self.max_width = int(self.A4_WIDTH_MM * self.DPI / 25.4)
        self.max_height = int(self.A4_HEIGHT_MM * self.DPI / 25.4)
    ...
    def create_sheet(self, 
        ...
        # 実際に必要なサイズを計算
        actual_width = min(self.max_width, layout_info['content_width'] + 2 * self.margin)
        actual_height = min(self.max_height, layout_info['content_height'] + 2 * self.margin + self.info_height)
        
        # 背景画像作成（実際のサイズ）
        sheet = Image.new('RGB', (actual_width, actual_height), 'white')
        draw = ImageDraw.Draw(sheet)
        
        # 画像を配置
        self._place_images(sheet, processed_images, film_format, layout_info)
        
        # 情報セクションを追加
        self._add_info_section(sheet, draw, info, actual_width, actual_height)
    ...
    def _calculate_layout(self, processed_images: List[Tuple[Image.Image, int]], 
        ...
        # 画像サイズの計算
        available_width = self.max_width - (2 * self.margin)
    ...
    def _place_images(self, sheet: Image.Image, 
                     processed_images: List[Tuple[Image.Image, int]],
                     film_format: FilmFormat, layout_info: Dict) -> None:
        """画像を配置"""
        current_row = 0
        current_col = 0
    ...
    def _add_info_section(self, sheet: Image.Image, draw: ImageDraw.Draw, 
        ...
        draw.line([(self.margin, info_y), 
                  (sheet_width - self.margin, info_y)], 
                 fill='black', width=2)
        
        # 情報を描画
        y_offset = info_y + 15  # 上部余白
